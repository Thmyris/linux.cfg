...
_gmtool_config_() {
    ...
    UNAMED_OPTIONS="username password proxy_address proxy_port proxy_username proxy_password license_server_address trusted_hosts"
...
_gmtool_admin_() {
    SUBCOMMANDS="clone create delete details edit factoryreset list logzip start stop stopall templates"
    ...
    if [[ $i -eq $COMP_CWORD ]]; then
        ...
        details)
            ...
            return
            ;;
        start|stop|delete|factoryreset|clone)
            if [[ $((i+1)) -eq $COMP_CWORD ]]; then
                _gmtool_vm_complete "$cur"
            fi
            return
